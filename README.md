...
2. **Built the data pipeline.** 949 cat photos across 5 breeds from Kaggle.
...
Ragdolls are 21.7% of the data, so accuracy is the wrong metric — a model that
always says "not ragdoll" is 78.3% accurate and completely useless. Everything is
...
| Model | Ragdoll F1 |
| --- | --- |
| MLP baseline | 0.52 (val, never stabilized) |
| resnet18 fine-tuned | **0.806** (test) |
...
Target is F1 ≥ 0.90, and 0.806 isn't that, which is why the site tells you the
